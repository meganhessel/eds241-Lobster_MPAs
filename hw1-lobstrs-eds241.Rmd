...
# Making a `reef` from `site`
tidydata <- rawdata %>% 
    mutate(reef = case_when(
        rawdata$site == "AQUE" ~ 'Arroyo Quemado',
        rawdata$site == "CARP" ~ 'Carpenteria', 
        rawdata$site == "MOHK" ~ 'Mohawk',
        rawdata$site == "IVEE" ~ 'Isla Vista',
        rawdata$site == "NAPL" ~ 'Naples'
    ))
# Making it a factor with the ordered wanted 
tidydata$reef <- factor(tidydata$reef, levels = custom_order)
...
spiny_counts <- tidydata %>% 
    ...
    summarise(obs_lob = sum(c(count, num_ao), na.rm = TRUE), # Number of lobsters observed 
...
spiny_counts <- spiny_counts %>% 
    mutate(mpa = case_when(
        spiny_counts$site == 'AQUE' ~ 'non_MPA',
        spiny_counts$site == 'CARP' ~ 'non_MPA', 
        spiny_counts$site == 'MOHK' ~ 'non_MPA',
        spiny_counts$site == 'IVEE' ~ 'MPA',
        spiny_counts$site == 'NAPL' ~ 'MPA'
))
